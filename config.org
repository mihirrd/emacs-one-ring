...
#+end_src
** Olivetti
Distraction-free writing mode: centers the buffer at a comfortable reading width and hides the fringes. Toggle with ~C-c o~.
#+begin_src emacs-lisp
...
#+end_src
** git-timemachine
Step through a file's git history buffer-locally. ~p~/~n~ moves between revisions, ~q~ quits.
#+begin_src emacs-lisp
